...
class ProTradingSystem:
    ...
    def _generate_entry_signals(self, signals_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.config.require_confluence:
            ...
            # Use simplified but enhanced original signals with ADX filtering
            buy_trigger = (
                basic_buy_setup &
                (signals_df['rsi'] > self.config.rsi_oversold) &  # Enhanced RSI thresholds
                (signals_df['rsi'] < self.config.rsi_overbought) &
                (signals_df['macd_line'] > signals_df['signal_line']) &
                signals_df.get('volume_vol_bull', True)
            )
            
            sell_trigger = (
                basic_sell_setup &
                (signals_df['rsi'] > self.config.rsi_oversold) &
                (signals_df['rsi'] < self.config.rsi_overbought) &
                (signals_df['macd_line'] < signals_df['signal_line']) &
                signals_df.get('volume_vol_bear', True)
            )
            
            # Apply ADX regime filtering if enabled
            if self.config.enable_regime_filter:
                adx_filter = (
                    ~signals_df['advanced_adx_choppy'] &  # Not choppy (ADX > 20)
                    ~signals_df['advanced_adx_strong_trending']  # Not extreme (ADX < 30)
                )
                buy_trigger = buy_trigger & adx_filter
                sell_trigger = sell_trigger & adx_filter
        ...
        # Apply minimum gap between signals (enhanced with config values)
